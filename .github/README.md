...
> [!IMPORTANT]
> **Changes here are overwritten on publish.**
>
> JavaScript in the [npm-element-types](https://github.com/process-co/npm-element-types) repo is synced from the internal monorepo (`proc-app`). The publish pipeline bundles the public surface of selected libraries. This repository only holds what is published to npm — edit **`process-co/element-types`** in the monorepo, then sync per **`AGENTS.md`**.
